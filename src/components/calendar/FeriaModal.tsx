...
interface Props {
  ...
  ferias: Feria[];
  onClose: () => void;
  onAssign: (agenteId: string, fechaInicio: string, fechaFin: string, motivo: string) => Promise<void>;
...
export function FeriaModal({ agents, ferias, onClose, onAssign, onDelete }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await onAssign(agenteId, fechaInicio, fechaFin, motivo);
      setAgenteId("");
      setFechaInicio("");
      setFechaFin("");
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-2xl flex flex-col max-h-[85vh] overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-col lg:flex-row flex-1 overflow-hidden min-h-0">
          ...
          {/* Fomulario */}
          <div className="p-4 sm:p-5 lg:w-5/12 bg-slate-50 border-b lg:border-b-0 lg:border-r border-slate-100 flex flex-col gap-4 overflow-y-auto">
            <h3 className="text-sm font-bold text-slate-700">Registrar Ausencia</h3>
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                >
                  ...
                  <option value="Licencia / Días solicitados">Licencia / Días solicitados</option>
              ...
              <button 
                type="submit"
                disabled={loading}
                className="mt-2 w-full flex items-center justify-center gap-2 bg-sky-600 hover:bg-sky-700 text-white font-semibold py-2 rounded-lg transition-colors disabled:opacity-50 shadow-sm cursor-pointer text-sm"
              >
                      ...
                      return (
                        <div key={f.id} className="flex items-center justify-between p-3 rounded-xl border border-sky-100 bg-sky-50/50 hover:bg-sky-50 transition-colors">
                          <div className="flex flex-col">
                            ...
                            <span className="text-[10px] text-slate-500 font-bold mt-0.5 bg-sky-100/50 text-sky-850 px-2 py-0.5 rounded-full w-fit">
                              {f.motivo || "Vacaciones"}
                            </span>
                          </div>
                          <button 
